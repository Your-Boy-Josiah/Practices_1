...
### New Commit =>🛠️ Key Technical Issues Resolved Today

Fixec all the below Errors and added also some new features

1. Path-to-RegExp / Express 404 Route Error:

2. Removed deprecated app.use('*', ...) wildcard handler that broke under modern Express/Node.js runtimes. Replaced with pathless fallback middleware app.use((req, res, next) => ...).

3. express-mongo-sanitize Read-Only Property Crash:

4. Resolved TypeError: Cannot set property query of #<IncomingMessage> which has only a getter by wrapping mongoSanitize.sanitize() to modify inputs in-place without reassigning req.query.

5. Mongoose Modern Async Pre-Save Hook Crash:

6. Resolved TypeError: next is not a function in Models/Users.js by removing legacy next callbacks from the asynchronous hook definition.

7. Multer Media Upload Integration:

8. Configured disk storage in Middleware/upload.js ensuring unique file names via timestamped suffixes.

9. Configured static serving via app.use('/uploads', express.static(...)) with Helmet cross-origin permissions enabled.
